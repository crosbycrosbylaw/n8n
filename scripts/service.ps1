...
switch ($action) {
  'start' { 
    if (!$procs) { 
      ...
      start_n8n_process ; start-sleep $HEALTH_INTERVAL
      ...
      if (!$running) {
        ...
        while (!$running -and ($count -lt $MAX_ATTEMPTS)) {
          ...
          if ($running) { write-output "[n8n] service started successfully" ; break } 
      ...
      write-output '[n8n] service is already running' 
    }
  }
  'stop' { 
  ...
  'status' {
    if ($procs) { $procs | write-output } else { write-output '[n8n] service is down' } 
  }
  'poll' {
