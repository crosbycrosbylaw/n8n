...
from automate.eserv.extract import *
...
if typing.TYPE_CHECKING:
    ...
    from automate.eserv.types.structs import DownloadInfo
...
def _process_accepted_response(
    ...
    if file_no is not None:
        return (f'untitled_{file_no}', content)
...
def _bypass_aspnet_form(
    ...
    try:
        ...
        raise DocumentDownloadError from e
...
def _process_response(
    ...
    if 'text/html' not in content['type']:
        ...
        raise DocumentDownloadError(message)
...
def download_documents(soup: BeautifulSoup) -> DownloadInfo:
    ...
    Returns:
        out (tuple[str | None, Path]):
            A tuple containing the extracted document name (if it exists, otherwise `None`) and a \
                `Path` object pointing to the directory where the downloaded documents were stored.

    Note:
        - The function expects extract_download_info() to return an object with 'name' and 'link' \
            attributes.
        - Downloaded files are saved with sanitized filenames (only alphanumeric characters and .,\
            _, -, space).
