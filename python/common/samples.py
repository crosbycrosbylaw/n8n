...
@dataclass
class Sample(DocumentInfo):
    text: str = field(default="", compare=False)
    parties: list[HumanName] = field(default_factory=list, compare=False)

    def customize(self, pick: Sequence[str] | None = None, omit: Sequence[str] = ()) -> DocumentInfo:
        info = DocumentInfo()
        generator = (f.name for f in fields(self) if f.name not in omit and (not pick or f.name in pick))
        for name in generator:
            setattr(info, name, getattr(self, name))
        return info
...
FA.append(
    Sample(
        href="https://illinois.tylertech.cloud/ViewDocuments.aspx?FID=9e4802ef-9954-4ed2-963b-4e50f0cbff98",
        name="Motion to Modify Maintenance RTF.pdf",
        desc="Candace R Boudreau vs. Christopher R Boudreau",
        parties=[HumanName(f"{n} R Boudreau") for n in ["Candace", "Christopher"]],
