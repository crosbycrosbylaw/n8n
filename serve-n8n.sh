...
start_server() {
    ...
    print_status "${YELLOW}" "Server started but health check failed. It may still be initializing."
...
stop_server() {
    if ! is_running; then
        print_status "${YELLOW}" "n8n server is not running"
        return 0
    fi
    ...
    if kill -TERM "${pid}" 2>/dev/null; then
        ...
        if kill -0 "${pid}" 2>/dev/null; then
            print_status "${YELLOW}" "Graceful shutdown failed, force killing..."
...
monitor_server() {
    ...
    print_status "${BLUE}" "Starting n8n server monitor..."
    ...
    while true; do
        if ! is_running; then
            ...
            print_status "${YELLOW}" "Server not running. Attempting restart (${restart_count}/${MAX_RESTARTS})..."
            ...
            if ! health_check; then
                print_status "${YELLOW}" "Health check failed"
...
show_status() {
    if is_running; then
        ...
        if health_check; then
            ...
            print_status "${YELLOW}" "Server process is running but not responding to health checks"
        fi
...
main() {
    ...
    case "${1:-start}" in
        start)
            if is_running; then
                print_status "${YELLOW}" "n8n server is already running"
                show_status
            else
                start_server
            fi
