...
create_user() {
    if ! id "${SERVICE_USER}" &>/dev/null; then
        ...
        print_status "${YELLOW}" "⚠️  User ${SERVICE_USER} already exists"
    fi
}
...
install_dependencies() {
    ...
    if [ "${has_runtime}" = false ]; then
        print_status "${YELLOW}" "⚠️  No Node.js runtime found. Installing Node.js..."
...
main() {
    ...
    echo
    print_status "${YELLOW}" "⚠️  Remember to configure firewall rules if needed"
}
